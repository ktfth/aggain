import { generateServiceTemplate, generateServiceTest } from '../templates/service.js';
import { mkdir, writeFile } from 'fs/promises';
import { logger } from '../utils/logger.js';
import path from 'path';
export async function generateServiceFiles(options, includeTests, projectPath) {
    const { name } = options;
    try {
        ...
        await writeFile(path.join(projectPath, 'src', 'services', `${name}.service.ts`), serviceContent);
        ...
        if (includeTests) {
            ...
            await writeFile(path.join(projectPath, 'src', '__tests__', 'services', `${name}.service.test.ts`), testContent);
        }
