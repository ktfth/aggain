...
export async function generateComponent(options) {
    ...
    try {
        ...
        // Salvar arquivo
        await writeFile(path.join(componentDir, `${nameLower}.ts`), content);
        logger.info({ name, type }, `Componente ${name} do tipo ${type} gerado com sucesso!`);
    }
    catch (error) {
        logger.error({ error, name, type }, `Erro ao gerar componente ${name} do tipo ${type}`);
        throw error;
    }
}
