...
import { SubmoduleOptions, generateSubmodule } from './generators/submodule.js';
import {
  userFieldSchema,
  validateAuthFramework,
  validateAuthOptions,
  validateAuthType,
  validateFramework,
  validateProjectName
...
import type { CrudOptions } from './templates/crud.js';
...
let adminServer: express.Application | null = null;

function startAdminServer(port: number = 3001) {
  if (!adminServer) {
    adminServer = express();
...
async function handleAuth(argv: any) {
  try {
    ...
    let userFields: z.infer<typeof userFieldSchema>[] = [];
    if (argv.userFields) {
      try {
...
async function handleComponent(argv: any) {
  try {
...
async function handleCrud(argv: any) {
  try {
    ...
    const framework = validatedFramework as 'express' | 'koa';
...
async function handleService(argv: any) {
  try {
...
yargs(hideBin(process.argv))
    ...
    handler: async (argv) => {
      try {
        const projectOptions: ProjectOptions = {
          ...
          database: argv.database as 'mongodb' | 'postgresql' | 'mysql' | undefined,
  ...
  .command({
    ...
    handler: async (argv) => {
      try {
        const options = argv as unknown as SubmoduleCommandOptions;
        const properties = JSON.parse(options.properties);
        const submoduleOptions: SubmoduleOptions = {
