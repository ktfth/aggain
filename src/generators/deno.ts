...
function generateDenoConfig(): string {
...
export async function generateDenoProject(options: ProjectOptions): Promise<void> {
  ...
  try {
    ...
    if (options.includeTests) {
      await writeFile(
        path.join(projectDir, 'tests/example_test.ts'),
        generateExampleTest()
      );
    }
