...
function generateEnvFile(options: ProjectOptions): string {
  return `# Application
...
function generateCustomErrors(): string {
...
function generateSwaggerConfig(): string {
...
function generateDatabaseConnection(database?: string): string {
  if (database === 'mongodb') {
...
function generateMongooseModel(): string {
...
function generateTypeORMEntity(): string {
...
function generateUserService(database?: string): string {
...
function generateUserController(database?: string): string {
  return `import { Request, Response, NextFunction } from 'express';
...
function generateUserRoutes(): string {
...
export async function generateExpressProject(options: ProjectOptions): Promise<void> {
...
.DS_Store`
    ...
    // Gerar config
    await writeFile(
      path.join(projectDir, 'src/config/index.ts'),
      generateConfig()
    );

    await writeFile(
      path.join(projectDir, 'src/config/swagger.ts'),
    ...
    // Gerar rotas
    await writeFile(
      path.join(projectDir, 'src/routes/health.ts'),
      generateHealthRoute()
    );
    ...
    if (hasDatabase) {
      await writeFile(
        path.join(projectDir, 'src/routes/auth.ts'),
        generateAuthRoutes()
      );

      await writeFile(
        path.join(projectDir, 'src/routes/user.ts'),
        generateUserRoutes()
      );
    }
    ...
    if (hasDatabase) {
      await writeFile(
        path.join(projectDir, 'src/controllers/AuthController.ts'),
        generateAuthController()
      );

      await writeFile(
        path.join(projectDir, 'src/controllers/UserController.ts'),
        generateUserController(options.database)
      );
    }
    ...
    if (hasDatabase) {
      ...
      await writeFile(
        ...
        generateUserService(options.database)
      );
    }
    ...
    if (hasDatabase) {
      await writeFile(
        path.join(projectDir, 'src/middlewares/auth.ts'),
    ...
    await writeFile(
      ...
      generateCustomErrors()
    );

    if (hasDatabase) {
      await writeFile(
        path.join(projectDir, 'src/utils/database.ts'),
        generateDatabaseConnection(options.database)
      );
    }
...
function generatePackageJson(options: ProjectOptions): object {
  ...
  const databaseDevDeps: Record<string, string> = {};
...
function generateDockerCompose(options: ProjectOptions): string {
  const hasDatabase = !!options.database;
